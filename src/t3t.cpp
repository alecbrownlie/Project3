...
void T3T::buildTree(ifstream & input, bool outputFlag){
...
void T3T::searchTree(ifstream & input){
    node* n;
    stringstream tempWord;
    double totalTime, finishTime, startTime = clock();
    while (!input.eof()) {
...
void T3T::printTreeHelper(node* t, ostream & out) const{
    if(t == nullptr)
        ...
        out << t->lkey << " " << t->lval[0]; //left is filled first
        for (int i = 1; i < t->lval.size(); i++)
            out << ", " << t->lval[i];
        out << endl;

        if (t->rkey == "") { //since right key is empty, there is no middle child
